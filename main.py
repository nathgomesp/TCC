...
def temp_medio(valor, planta):
    ...
    if faixa[0] <= valor <= faixa[1]:
        ...
        return (valor - faixa[0]) / (meio - faixa[0]) if valor <= meio else (faixa[1] - valor) / (faixa[1] - meio)
    return 0

def temp_quente(valor, planta):
    faixa = faixas_temperatura[planta]["quente"]
    return max(0, min(1, (valor - faixa[0]) / (faixa[1] - faixa[0]))) if valor >= faixa[0] else 0
